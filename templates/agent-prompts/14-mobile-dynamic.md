# Terminal: mobile-dynamic-analysis-auditor (Phase 2 — parallel)
...
brew install --cask android-platform-tools 2>/dev/null  # adb
...
- `AUDIT_SKILLS_PATH`
- A reachable instrumented device:
  - **Android:** rooted emulator OR physical device with `frida-server` running, `adb devices` non-empty
  - **iOS:** jailbroken device with Frida via Sileo OR Frida-gadget-repackaged build
- If `frida-ps -U` returns nothing → write `BLOCKED: no instrumented device available` and exit.
