# Terminal: supabase-auth-auditor (Phase 2 — parallel)
...
brew install jq postgresql 2>/dev/null  # jq for settings parse, psql for audit log query
claude --dangerously-skip-permissions
```

## Required env

- `SUPABASE_PROJECT_REF` — e.g. `abcdefghijklm`
- `SUPABASE_ANON_KEY` — publishable / anon key for `/auth/v1/settings`
- `SUPABASE_DB_URL` — optional; needed for `auth.audit_log_entries` + version queries
- `SUPABASE_ACCESS_TOKEN` — optional; PAT for management API (`/v1/projects/<ref>/config/auth`)
- `AUDIT_SKILLS_PATH`
