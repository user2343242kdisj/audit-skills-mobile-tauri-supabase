# Terminal: setup-agent (run ONCE before any audit)

Bootstraps the audit toolchain inside `~/desktop/travus`: installs the `exec-agent` wrapper on PATH, clones audit-skills if missing, creates `audit-reports/`, hardens `.gitignore`, and scaffolds `.audit-env` with placeholders for you to fill in.
...
(No `source .audit-env` yet — this agent creates it.)
