...
PRE-WORKFLOW: Resolve paths + (optionally) signing secrets

Most of this agent operates on local files. The Updater and Binary
Hardening sections benefit from comparing against trusted values from
1Password — but if those reads fail, the agent should still complete
the local-file portions and note the comparisons as "skipped: op read
failed".
