# Terminal: mobile-static-analysis-auditor (Phase 2 — parallel)
...
brew install apktool jadx 2>/dev/null
brew install --cask android-platform-tools 2>/dev/null  # apksigner, aapt2
brew install xmlstarlet libxml2 2>/dev/null              # xmllint
brew install ripgrep jq 2>/dev/null
# class-dump / otool / plutil / codesign ship with macOS + Xcode CLT
...
- `AUDIT_SKILLS_PATH`
- `MOBSF_API_KEY` (optional — only if running MobSF Docker locally)
