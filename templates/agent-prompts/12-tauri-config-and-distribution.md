# Terminal: tauri-config-and-distribution-auditor (Phase 2 — parallel, MERGED)

This single terminal runs three coordinated audits in one Claude Code session:
- **Section A — CSP / WebView** (role: `tauri-csp-webview-auditor`)
- **Section B — Updater** (role: `tauri-updater-auditor`)
- **Section C — Binary hardening** (role: `tauri-binary-hardening-auditor`)

All three roles share the same `tauri.conf.json` and `Cargo.toml` surface, so they audit in parallel and write a single combined report.
...
docker --version >/dev/null 2>&1 || echo "WARN: docker not available; checksec section will be skipped"
